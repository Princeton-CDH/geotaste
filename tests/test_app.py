...
def test_showhide_components(dash_duo):
    ...
    # modal open
    assert (
        dash_duo.find_element('#welcome-modal .modal-title').text
        == WELCOME_HEADER
    )
    ...
    def test_button_showhide_l(button_showhide_ids, close=False):
        for btn_id in button_showhide_ids:
            ...
            assert dash_duo.wait_for_style_to_equal(
                body_id, 'display', 'block'
            )

            if close:
                dash_duo.multiple_click(btn_id, 1)
                assert dash_duo.wait_for_style_to_equal(
                    body_id, 'display', 'none'
                )
    ...
    member_cards1 = [
        x
        for x in button_showhide_ids
        if '-MP-' in x and 'Card' in x and 'Filter_1' in x
    ]
    member_cards2 = [
        x
        for x in button_showhide_ids
        if '-MP-' in x and 'Card' in x and 'Filter_2' in x
    ]
    book_cards1 = [
        x
        for x in button_showhide_ids
        if '-BP-' in x and 'Card' in x and 'Filter_1' in x
    ]
    book_cards2 = [
        x
        for x in button_showhide_ids
        if '-BP-' in x and 'Card' in x and 'Filter_2' in x
    ]
...
def test_suites(dash_duo, done=True):
    ...
    if done:
        ...
        dash_duo.wait_for_contains_text(
            '#store_desc-Filter_2', 'United States'
        )
        dash_duo.wait_for_contains_text('#tblview', 'comparing')

        # clear right by clicking filter clear
        dash_duo.multiple_click(
            '#button_clear-MemberNationalityCard-MP-Filter_2', 1
        )
        _nap()
        dash_duo.wait_for_text_to_equal(
            '#store_desc-MemberNationalityCard-MP-Filter_2', BLANK
        )
        ...
        dash_duo.wait_for_text_to_equal(
            '#store_desc-MemberNationalityCard-MP-Filter_1', BLANK
        )

        # panel filter
        dash_duo.multiple_click(
            '#button_showhide-MemberNationalityCard-MP-Filter_1', 1
        )
        _nap()
        dash_duo.wait_for_contains_text(
            '#graph-MemberNationalityCard-MP-Filter_1', '4031'
        )
        ...
        dash_duo.multiple_click(
            '#button_showhide-MembershipYearCard-MP-Filter_1', 1
        )
        _nap()
        dash_duo.wait_for_contains_text(
            '#input_start-MembershipYearCard-MP-Filter_1', '1919'
        )
        dash_duo.multiple_click('#test_suite_btn6', 1)
        _nap()
        dash_duo.wait_for_contains_text(
            '#input_start-MembershipYearCard-MP-Filter_1', '1932'
        )
        dash_duo.wait_for_contains_text(
            '#input_end-MembershipYearCard-MP-Filter_1', '1939'
        )

        # dash_duo.multiple_click('#button_showhide-MembershipYearCard-MP-Filter_1', 1)
        input1 = dash_duo.find_element(
            '#input_start-MembershipYearCard-MP-Filter_1'
        )
        input2 = dash_duo.find_element(
            '#input_end-MembershipYearCard-MP-Filter_1'
        )
        input1.send_keys(
        ...
        _nap()
        dash_duo.wait_for_contains_text(
            '#input_start-MembershipYearCard-MP-Filter_1', '1920'
        )
        dash_duo.wait_for_contains_text(
            '#input_end-MembershipYearCard-MP-Filter_1', '1930'
        )
        _nap()
        dash_duo.multiple_click(
            '#button_clear-MembershipYearCard-MP-Filter_1', 1
        )
        _nap()
        dash_duo.wait_for_text_to_equal(
            '#store_desc-MembershipYearCard-MP-Filter_1', BLANK
        )
        dash_duo.wait_for_text_to_equal('#store_desc-Filter_1', BLANK)
    ...
    dash_duo.wait_for_text_to_equal(
        '#store_desc-MemberNameCard-MP-Filter_1', 'James Joyce'
    )
    dash_duo.multiple_click('#button_clear-MemberNameCard-MP-Filter_1', 1)
    _nap()
    dash_duo.wait_for_text_to_equal(
        '#store_desc-MemberNameCard-MP-Filter_1', BLANK
    )
    dash_duo.wait_for_text_to_equal('#store_desc-Filter_1', BLANK)
...
def test_query_strings(dash_duo):
    ...
    for host in TEST_HOSTS:
        ...
        driver.get(
            f'{host}?tab=table&tab2=book&member_gender=Female&member_gender2=Male'
        )
        _nap()
