...
def biasedseries(bias_n=1, bias_fac=0.5):
...
def test_analyze_contingency_tables():
    ...
    output_df = analyze_contingency_tables(
        vals1, vals2, min_p=MIN_P, signif=True
    )

    # Check that the output DataFrame has the correct columns
    assert set(output_df.columns) == set(
        cols
    ), f'Output DataFrame should contain columns for {cols}'

    # Check that the DataFrame is sorted in ascending order by 'odds_ratio'
    assert (
        output_df['odds_ratio'].sort_values() == output_df['odds_ratio']
    ).all(), (
        'Output DataFrame should be sorted in ascending order by odds_ratio'
    )

    # Check that the DataFrame contains only rows with 'fisher_exact_p' <= MIN_P
    assert (
        output_df['fisher_exact_p'] <= MIN_P
    ).all(), "Output DataFrame should only contain rows with 'fisher_exact_p' <= MIN_P"
    ...
    assert r1.fisher_exact_p < 0.05
    assert r2.fisher_exact_p < 0.05
...
def test_geodist():
    ...
    assert (
        abs(geodist((48.8566, 2.3522), LATLON_SCO)) > 1
    ), 'Test case 1 failed'
    # Test with the same location (distance should be 0).
    assert (
        geodist((48.8566, 2.3522), (48.8566, 2.3522)) == 0
    ), 'Test case 2 failed'
...
def test_get_distinctive_qual_vals():
    ...
    # Run the function
    output = get_distinctive_qual_vals(
        dfL, dfR, cols=['member_title', 'member_nationalities']
    )
    ...
    assert all(
        col in output.columns for col in columns
    ), 'Output DataFrame is missing expected columns.'

    # Check the 'col' and 'col_val' column have expected values
    assert (
        output['col'].nunique() == 2
    ), "'col' column does not have 2 unique values."

    # Check the 'odds_ratio' column values
    assert all(
        np.isnan(output['odds_ratio']) == False
    ), "'odds_ratio' column contains NaN values."
    ...
    assert all(
        output['count_L'] != 0.0
    ), "'count_L' column values are all 0.0."
    assert all(
        output['count_R'] != 0.0
    ), "'count_R' column values are all 0.0."

    output2 = get_distinctive_qual_vals(
        dfL,
        dfR,
        cols=['member_title', 'member_nationalities'],
        drop_duplicates='member',
    )
    output3 = get_distinctive_qual_vals(
        dfL,
        dfR,
        cols=['member_title', 'member_nationalities'],
        drop_duplicates=['member'],
    ...
    # Run the function
    output4 = get_distinctive_qual_vals(
        dfL,
        dfR,
        cols=[
            'member_title',
...
def test_describe_comparison():
    ff = ComparisonFigureFactory(
        {'member_gender': ['Male']}, {'member_gender': ['Female']}
    )
...
def test_group_overlaps():
    df = pd.DataFrame(
        [
            {'group': 'shoe', 'part': 'foot'},
            {'group': 'shoe', 'part': 'toe'},
            {'group': 'hat', 'part': 'head'},
        ]
    )
    assert list(group_overlaps(df.group, df.part, ['head'])) == [
        False,
        False,
        True,
    ]
    assert list(group_overlaps(df.group, df.part, ['toe'])) == [
        True,
        True,
        False,
    ]
    assert list(group_overlaps(df.group, df.part, ['foot'])) == [
        True,
        True,
        False,
    ]
    assert list(group_overlaps(df.group, df.part, ['xxx'])) == [
        False,
        False,
        False,
    ]
    ...
    assert list(
        group_overlaps(df.group, df.part, ['head'], allow_none=True)
    ) == [False, False, True, None]
    assert list(
        group_overlaps(df.group, df.part, ['head'], allow_none=False)
...
def test_overlaps():
    ...
    df.loc[len(df)] = {'group': 'desk', 'part': ''}
    assert list(overlaps(df.part, ['head'], allow_none=True)) == [
        False,
        False,
        True,
        None,
    ]
    assert list(overlaps(df.part, ['head'], allow_none=False)) == [
        False,
        False,
        True,
        False,
