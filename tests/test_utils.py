...
def test_make_counts_df():
    series = pd.Series(
        ['apple', 'banana', 'apple', 'orange', 'banana'], name='fruits'
    )
...
def test_combine_LR_df():
    ...
    odf = combine_LR_df(
        dfL, dfR, colname='colname', colval_L='colL', colval_R='colR'
    )
...
def test_selectrename_df():
    ...
    col2col = {'A': 'X', 'B': 'Y'}
    assert selectrename_df(df, col2col).equals(
...
def test_qualquant_series():
    assert (
        qualquant_series([1, 2, 3, 4, 5], True) == pd.Series([1, 2, 3, 4, 5])
    ).all()
    assert (
        qualquant_series(['1', '2', '3', '4', '5'], False)
        == pd.Series(['1', '2', '3', '4', '5'])
    ).all()
...
def test_oxfordcomma():
    ...
    assert (
        oxfordcomma(['apple', 'banana', 'orange'])
    ...
    assert (
        oxfordcomma(['apple', 'banana', 'orange'], op='or')
    ...
    assert oxfordcomma(['1', '2', '3'], repr=str) == '1, 2, and 3'
    assert (
        oxfordcomma(['apple', 'banana', 'orange'], repr=str)
        == 'apple, banana, and orange'
    )
...
def test_read_config_json():
    ...
    with tempfile.TemporaryDirectory() as tdir:
        ...
        with open(badfile, 'w') as of:
            of.write('gobbledygook')
        try:
            read_config_json(badfile)
            assert (
                False
            ), 'ought to throw exception for reading unknown json file'
        except Exception:
            assert True, 'success'
