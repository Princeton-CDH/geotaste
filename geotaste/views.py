...
def ArrondTableView(ff, Lstr='', Rstr=''):
    ...
    def getdesc(signif_df, filter_desc, side='left'):
        ...
        descs=['',f'The {side}-hand group is...']
    ...
    desc_L=getdesc(signif_more_L, Lstr,side='left') if len(signif_more_L) else ''
    desc_R=getdesc(signif_more_R, Rstr,side='right') if len(signif_more_R) else ''
