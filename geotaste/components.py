...
class BaseComponent(DashComponent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        name = (
            name
            if name
            else (self.name if self.name else self.__class__.__name__)
        )
        if name_context and name_context != name:
            name = f'{name}-{name_context}'
        if name_add_uuid:
            name = name.split('_', 1)[0] + '_' + uid(4)
    ...
    def cards_with_attr(self, attrname: str):
        return [card for card in self.subcomponents if hasattr(card, attrname)]
    ...
    def get_content(self, params=None, **kwargs):
        ...
        return dbc.Container(
            ...
            else [card.layout(params) for card in self.subcomponents]
        )
...
class FilterComponent(BaseComponent):
    ...
    def ff(self, filter_data={}, selected: list = [], **kwargs):
        ...
        logger.trace(
            f'[{self.name}.ff({filter_data}, {selected}, **{kwargs})]'
        )

        filter_data = {
            k: v
            for k, v in filter_data.items()
            if k not in set(selected) and k != self.key
        }

        return self.figure_factory(
            filter_data=filter_data, selected=selected, **kwargs
        )
    ...
    def plot(
        ...
        **kwargs,
        ...
        if min_val is not None:
            ff.min_series_val = min_val
        if max_val is not None:
            ff.min_series_val = max_val
        # return ff.fig
        return ff.plot(**kwargs)

    @cached_property
    def series(self):
        return self.ff().series
...
class FilterCard(FilterComponent):
    ...
    def header(self):
        ...
        return dbc.CardHeader(
            [self.showhide_btn, self.header_btn, self.contrast_btn],
    ...
    def contrast_btn(self):
        return dbc.Button(
            ...
            style={
                'display': 'none' if not self.show_contrast_btn else 'inline'
            },
        )
    ...
    def component_callbacks(self, app):
        ...
        app.clientside_callback(
            ClientsideFunction(
                namespace='clientside', function_name='toggle_showhide_btn'
            ),
            [
        ...
        app.clientside_callback(
            ...
            [Input(self.store, 'data'), Input(self.body, 'is_open')],
            prevent_initial_call=True,
        )
...
class FilterPlotCard(FilterCard):
    ...
    def plot(self, filter_data={}, selected=[], **kwargs):
        return plot_cache(
            self.figure_factory,
            serialize(
                [filter_data, selected, {**kwargs, **{'color': self.color}}]
            ),
        )
    ...
    def component_callbacks(self, app):
        ...
        def cache_graph_json(
            is_open, json_now, my_filter_data, panel_filter_data
        ):
            if not is_open or json_now:
                raise PreventUpdate
            newdata = {
                k: v
        ...
        def panel_data_updated(
            ...
            if not _clicked_open_1 and not _clicked_open_2:
                raise PreventUpdate
            logger.debug(
                f'triggered by {ctx.triggered_id}, with panel_filter_data = {panel_filter_data} and my_filter_data = {my_filter_data}'
            )
            ...
            ofig_json_gz_str = self.plot(newdata, selected=my_filter_data)
            logger.debug(
                f'Assigning a json string of size {sys.getsizeof(ofig_json_gz_str)} compressed, to self.store_json'
            )
        ...
        app.clientside_callback(
            ClientsideFunction(
                namespace='clientside', function_name='decompress_json_gz'
            ),
...
class FilterSliderCard(FilterPlotCard):
    ...
    def component_callbacks(self, app):
        ...
        def graph_selection_updated2(
            graph_selected_data, btn_clk, start_value, end_value, panel_data
        ):
            ...
            if ctx.triggered_id == self.graph.id:
                ...
                if not seldata or not seldata.get(self.key):
                    raise PreventUpdate
                    ...
                    html.Ul(
                        [
                            html.Li(
                                html.A(
                                    children='Credits',
                                    href='https://shakespeareandco.princeton.edu/lab-credits/',
                                    target='_blank',
                                )
                            ),
                            html.Li(
                                html.A(
                                    children='Project',
                                    href='https://shakespeareandco.princeton.edu',
                                    target='_blank',
                                )
                            ),
                            html.Li(
                                html.A(
                                    children='Code',
                                    href='https://github.com/Princeton-CDH/geotaste',
                                    target='_blank',
                                )
                            ),
                        ]
