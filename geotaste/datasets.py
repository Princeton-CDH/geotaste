...
class LandmarksDataset(Dataset):
    ...
    def data(self):
        df = super().data
        df['arrond_id'] = [
            determine_arrond(lat, lon) for lat, lon in zip(df.lat, df.lon)
        ]
...
def get_member_id(uri):
    return (
        uri.lower().split('/members/', 1)[1][:-1] if '/members/' in uri else ''
    )
...
class MembersDataset(Dataset):
    ...
    cols_sep: list = ['nationalities', 'membership_years']
    cols_q = ['birth_year', 'death_year', 'membership_years']
    cols_pref = [
        'member',
        'member_nicename',
    ]
...
class DwellingsDataset(Dataset):
    ...
    def data(self):
        ...
        df['dwelling'] = [
            self.sep.join(str(x) for x in l) for l in df[self.cols_id].values
        ]
        df['arrond_id'] = df['arrrondissement'].apply(
            lambda x: '' if not x else str(int(x))
        )
        return postproc_df(df, cols_pref=self.cols_pref)
...
class BooksDataset(Dataset):
    ...
    def data(self):
        df = super().data
        df['book'] = df.uri.apply(
            lambda x: x.split('/books/', 1)[1][:-1] if '/books/' in x else ''
        )
        return postproc_df(df, cols_pref=self.cols_pref)
...
class EventsDataset(Dataset):
    ...
    cols_sep: list = ['member_uris', 'member_names', 'member_sort_names']
    ...
    def data(self):
        ...
        df['start_year'] = pd.to_numeric(
            [estr[:4] for estr in df['start_date'].apply(str)], errors='coerce'
        )
        df['start_month'] = pd.to_numeric(
            [
                x[5:7]
                if len(x) >= 7 and x[:4].isdigit() and x[4] == '-'
                else None
...
class MiniEventsDataset(Dataset):
    ...
    def data(self):
        ...
        for i, row in EventsDataset().data.iterrows():
            if self.only_borrows and row.event_type != 'Borrow':
                continue
...
class CombinedDataset(Dataset):
    ...
    def gen(self, save=True, progress=True, frac=None):
        ...
        memberdwellings = dfmembers.merge(
            dfdwellings, on='member', how='outer'
        )
        odf = memberdwellings.merge(
            bookevents, on='member', how='outer'
        ).fillna('')
...
def hover_tooltip(row):
    mrange_start = (
        f"{int(min(row.member_membership)) if row.member_membership else ''}"
    )
    mrange_end = (
        f"{int(max(row.member_membership)) if row.member_membership else ''}"
    )
    mrange = (
        f'{mrange_start} – {mrange_end}'
        if mrange_start != mrange_end
        else mrange_start
    )
    return f"""
...
def prune_when_dwelling_matches(df, progress=True):
    ...
    def declare_exact_miss(xdf):
        logger.trace(
            f'for event {e}, a certain match was found, with {len(xdf)} possibilities'
        )
        for ii in xdf.index:
    ...
    def declare_ambiguous(xdf, caveats=[]):
        for ii in xdf.index:
            probtype = (
                'Colens'
                if not len(xdf[xdf.dwelling_start != ''])
                else 'Raphael'
            )
    ...
    def find_exact_matches(xdf):
        ...
        match = xdf[
            [
                (
                    is_fuzzy_date_seq(d1, e1, d2)
                    or is_fuzzy_date_seq(d1, e2, d2)
                )
                for (d1, d2) in zip(xdf.dwelling_start, xdf.dwelling_end)
            ]
        ]
        logger.trace(
            f'found {len(match)} exact matches for {(e1, e2)} with options {list(zip(xdf.dwelling_start, xdf.dwelling_end))}'
        )
        return match
    ...
    def declare_heuristic_miss(xdf, htype=''):
        for ii in xdf.index:
            matchtyped[
                ii
            ] = f'Heuristic (excl.{" by "+htype if htype else ""})'
            matchfoundd[ii] = False
            excludedd[ii] = True

    # for every event...
    for e, edf in tqdm(
        df_event.groupby('event'),
        total=df_event.event.nunique(),
        desc='Locating events',
        ...
        if not len(edf):
            logger.trace(
                f'for event {e}, no dwelling possibilities because empty dwellings'
            )
            continue
        ...
        if len(edf_certain):
            logger.trace(
                f'for event {e}, certainty is possible, with {len(edf_certain)} possibilities'
            )
            ...
            if len(edf_match):
                logger.trace(
                    f'for event {e}, a certain match was found, with {len(edf_match)} possibilities'
                )
        ...
        if not len(edf):
            logger.trace(
                f'for event {e}, only a dispreferred address remained; allowing'
            )
            edf = edf0
        elif len(edf) != len(edf0):
            declare_heuristic_miss(
                edf0[~edf0.index.isin(edf.index)], htype='dispref'
            )
        ...
        edf = edf[
            [
                get_dist_from_SCO(lat, lon) < 50
                for lat, lon in zip(edf.lat, edf.lon)
            ]
        ]
        if not len(edf):
            logger.trace(
                f'for event {e}, only non-Parisian places remaining; allowing'
            )
            edf = edf0
        elif len(edf) != len(edf0):
            declare_heuristic_miss(
                edf0[~edf0.index.isin(edf.index)], htype='distance'
            )
            caveats.append('-distance')

        # otherwise, declare ambiguous?
        logger.trace(
            f'for event {e}, still no matches found. using all {len(edf)} possible indices'
        )
