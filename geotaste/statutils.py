...
def analyze_contingency_tables(
    ...
    index_name = (
        '__'.join(sorted(list(set([vals1.name, vals2.name]))))
        if not index_name
        else index_name
    )
    ld = []
    ...
    df = (
        df.sort_values(sort_by, ascending=sort_asc)
        if sort_by and len(df)
        else df
    )
    return df


def iter_contingency_tables(
    vals1: 'Iterable',
    vals2: 'Iterable',
    uniqvals: 'Iterable' = [],
    min_count: int = None,
...
def filter_signif(df, p_col=None, min_p=MIN_P):
    ...
    if min_p is None:
        return df
    if p_col is None:
        p_col = first([c for c in df if c == 'pvalue' or c.endswith('_p')])
    if not p_col:
        return df
...
def geodist(latlon1, latlon2, unit='km'):
    ...
    try:
        dist = distfunc((latlon1[1], latlon1[0]), (latlon2[1], latlon2[0]))
...
def describe_comparison(comparison_df, lim=10, min_fac=1.1):
    ...
    def get_list_desc(xdf, LR='L'):
        ...
        for i, row in xdf.iterrows():
            if min_fac and row.odds_ratio_pos < min_fac:
                continue
            pL = row[f'perc_{LR}']
            pR = row[f'perc_{LR2}']
            cL = row[f'count_{LR}']
...
def filter_series_by_value_count(series, n):
    ...
    filtered_series = series[
        series.isin(value_counts[value_counts >= n].index)
    ]
...
def compare_series(s1, s2, min_count=PREDICT_MIN_COUNT) -> pd.DataFrame:
    ...
    s2 = filter_series_by_value_count(
        no_null_series(flatten_series(s2)), min_count
    )
    s1, s2 = s1[s1.isin(s2)], s2[s2.isin(s1)]
    return analyze_contingency_tables(s1, s2, index_name='val').reset_index()
...
def postprocess_comparisons(iter_of_dfs: Iterable[pd.DataFrame]):
    ...
    # alldf=alldf[alldf.fisher_exact!=0] # both must have counts?
    alldf['odds_ratio_log'] = alldf['odds_ratio'].apply(np.log10)
    alldf['odds_ratio_pos'] = alldf['odds_ratio'].apply(
