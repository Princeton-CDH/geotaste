...
def get_distinctive_qual_vals(
    ...
    if cols is None:
        cols = list(set(dfL.columns) & set(dfR.columns))
    ...
    for col in cols:
        try:
            ...
            s1 = qualquant_series(
                flatten_series(dfLnow[col]), quant=False, drop_empty=drop_empty
            )
            s2 = qualquant_series(
                flatten_series(dfRnow[col]), quant=False, drop_empty=drop_empty
            )
            ...
            ).assign(col=col, comparison_scale=' '.join(dedupby))
            o.append(coldf)
