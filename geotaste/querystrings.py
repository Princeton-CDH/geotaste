...
def format_query_human(sname, svals):
    ...
    if len(svals) == 1:
        return (
            f'{humancol(sname)} is {"not " if is_neg else ""}{repr(svals[0])}'
        )
    ...
    query = sep.join(
        [
            (
                ...
                if not human
                else format_query_human(sname, svals)
            )
...
def format_query_pandas(
    sname, svals, groupby: Optional[str] = None, fname='contains'
):
    """
    ...
    if to_set(svals) == {'*'} or to_set(svals) == {'~'}:
        return format_boolean_query_pandas(
            sname, groupby, is_neg='~' in to_set(svals)
        )
...
def preprocess_for_possible_opening_negation(svals, symbol='~'):
    ...
    if not is_listy(svals):
        ...
        return preprocess_for_possible_opening_negation(
            svals[0], symbol=symbol
        )
    elif svals[0] == symbol:
        svals = svals[1:]
        is_neg = True
